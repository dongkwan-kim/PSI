...
Most codes are adopted from
    - https://github.com/GraphCL/PyGCL/blob/main/GCL/augmentors/functional.py
    - https://github.com/GraphCL/PyGCL/blob/main/GCL/utils.py
"""
...
def dropout_feature(x: torch.FloatTensor, drop_prob: float) -> torch.FloatTensor:
    # Fixed: p = 1 - drop_prob --> p = drop_prob
    return F.dropout(x, p=drop_prob)
...
def add_edge(edge_index: torch.Tensor, ratio: float, edge_weight=None) -> torch.Tensor:
    num_edges = edge_index.size()[1]
    num_nodes = edge_index.max().item() + 1
